...
const ChatHistory = ({ chatState, dispatch }: any) => {
  ...
  const toggleChatSize = () => {
    dispatch({ type: 'TOGGLE_SHRINK' });
    const chatHistDiv = document.querySelector('.chathist');
    // 其他逻辑
    console.log(isShrink);
    if (isShrink) {
      // 扩大时设置更大的宽度
      chatHistDiv?.classList.add('chat-width-transition');
      setChatWidth(1068); // 或者你可以设置其他宽度值
    } else {
      // 缩小时设置初始宽度
      chatHistDiv?.classList.add('chat-width-transition');
      setChatWidth(450);
  ...
  return (
    <div className="chathisbox">
      <div className="chathist" style={{ width: chatWidth }}>
        <div className="boxheader">
          ...
          <div className="btncon">
            ...
            />
            <Button shape="circle" type="secondary" icon={<IconPlus />} />
          </div>
        </div>
