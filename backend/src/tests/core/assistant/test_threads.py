...
async def test_threads_run_stateful_tool():
    ...
    assistant = Assistants.create(
        ...
        instructions="Use swe tool auto fix code files",
        tools=[{"type": "swe_tool"}],
    )
    print(assistant.id)
    # 运行 Threads 对象
    result = await threads.run(
        assistant.id, "Use SoftWare Engineer Agent swe tool auto fix code files."
    )
    print(result)

    result = await threads.run(
        assistant.id,
        "the repo url is https://github.com/YORG-AI/Open-Assistant",
        goto="stage_1",
    )
    print(result)

    result = await threads.run(
        assistant.id, "add helloworld feature to readme", goto="stage_2"
    )
    print(result)

    result = await threads.run(
        assistant.id, "focus_files_name_list = [README.md]", goto="stage_3"
    )
    print(result)
